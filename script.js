    ...
    songs: [
      {
        ...
        video: "https://www.youtube.com/embed/STG7w6h1IrM",
      },
      {
...
function loadClue() {
  ...
  questionText.innerText = `🎧 Listen to the song and guess the song name or artist! (Stage ${currentStage + 1})`;
  clueContainer.appendChild(questionText);
    ...
    audioElement.addEventListener("play", () => {
      setTimeout(() => {
        audioElement.pause();
        console.log("Audio file stopped after 2 seconds.");
      }, 2000);
    });
...
function checkAnswer() {
  let guess = document.getElementById("answer").value.toLowerCase().trim();
  let song = currentGenre.songs[currentStage];
  ...
  if (isCloseMatch(normalizedGuess, correctTitle) || isCloseMatch(normalizedGuess, correctArtist)) {
      currentStage++;
      document.getElementById("message").innerText = "✅ Correct! Unlocking next stage...";
  ...
  function isCloseMatch(userInput, correctAnswer) {
    ...
    return distance <= Math.ceil(maxLen * 0.3) || correctAnswer.includes(userInput);
  }
